...
response_flag=$(python ./file_availibility_in_api.py $REQUEST 2>&1)

echo $response_flag

if [[ "$response_flag" == "False" ]];
then
	echo "Video Id is not Valid";
	exit 0
else
	echo $REQUEST > $1.json
	cat $1.json
	...
	rm -rf $1.json
	echo "Congratulations VideoId is valid and the video is downloaded";
